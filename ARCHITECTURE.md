...
| MAC | OCB (no BSS); A-MPDU packing of QoS Data MPDUs with NDP BlockAck selective retransmission, NDP Ack / NDP CTS responses, RTS protection, RID, NAV, PHY-driven CCA, station identification frames (EtherType 0x88B5), per-peer rate control (`rate.rs`: EWMA success per MCS, one-step probing with back-off, retry step-down, SNR-bounded) | The only OCB liberty in the NDP path is the 9-bit partial AID derived from the MAC address; rate adaptation is implementation-defined in 802.11 |
