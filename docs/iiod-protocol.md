...
`192.168.2.1`, TCP port **30431**. libiio's `ip:` backend speaks this text protocol;
`s2g-sdr-pluto` implements it directly in Rust (no native dependencies), following
libiio `libiio-v0` `iiod-client.c` / `iiod/parser.y` / `iiod/ops.c`.
...
| Command | Reply |
|---|---|
| `VERSION\r\n` | retcode line; newer iiod versions follow it with version text |
...
| `CLOSE <dev>\r\n` | retcode |
| `READBUF <dev> <bytes>\r\n` | loop: retcode line (= bytes in this chunk, negative ⇒ error), then, **only with the first chunk after OPEN** (`thd->new_client` in iiod/ops.c), a hex mask line (`%08x…\n`), then that many raw binary bytes. Chunks repeat until `<bytes>` have been delivered; a retcode of 0 is sent only after a *partial* delivery (or on error) and ends the read. The virtual Pluto (`s2g-virtual-pluto`) reproduces this framing |
...
- TX stream device `cf-ad9361-dds-core-lpc`: output `voltage0`/`voltage1` i16 I/Q
  (the DAC uses the upper 12 bits: shift left by 4); interleaved
