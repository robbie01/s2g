...
| Good-neighbour frame filter | Self-consistent | Unit tests on crafted IPv4/IPv6/ICMPv6/extension-header frames; not yet exercised against a real OS stack, so a protocol that hides behind an unlisted port or EtherType simply passes (the filter is deny-listed, not allow-listed, above the IPv6/EtherType level) |
