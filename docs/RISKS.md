...
| Per-peer rate control | Self-consistent, simulated | Unit tests with synthetic success ceilings, a two-node PHY simulation at 30 dB / 11 dB SNR, and a link-level simulation (`tests/rate_sim.rs`) with the PHY's measured PER-vs-SNR curves over static, shadowed, Rician/Rayleigh and stepped channels, against which the constants were tuned (2026-09-03: 99 % of the best fixed rate on average, 80 % worst case). The SNR table is this PHY in AWGN loopback in the receiver's reported units; the hint assumes a roughly symmetric link; real fading statistics remain unmeasured |
