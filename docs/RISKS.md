...
- **External**: matched against a signal or decoder from outside this repository.
- **Self-consistent**: TX and RX agree with each other and with this reading of
  the spec; a systematic misreading would be invisible.
...
| Traveling pilots (TX) | Self-consistent | Loopback decodes; positions transcribed from Table 23-23 |
| MCS 3–7 (16-QAM … 64-QAM 5/6) | External | Baby-monitor S1G_LONG data PPDUs (MCS 3–7) decode byte-exact (2026-09-02) |
| MCS 8, 11 (256/1024-QAM) | Self-consistent | Loopback only; no capture uses them |
| LDPC (all rates, all block lengths) | Self-consistent, audited | Annex F matrices verified by H·cᵀ = 0; PPDU process (shortening / puncturing / repetition / extra symbol / tone mapper) round-trips. No external LDPC S1G signal exists in the captures. `scripts/matlab_vectors.m` generates reference waveforms if WLAN Toolbox is available. The PPDU process (Table 19-16 codeword selection, Eq 19-37..19-42, S1G N_pld/N_avbits, shortening/puncturing/repetition placement, D_TM = 4) was re-derived from the text on 2026-09-02 with no discrepancy |
...
| CCA energy / preamble / mid-packet detect | Self-consistent | Thresholds are dBm; without a calibration offset the receiver derives one from its measured noise floor (assumed −104 dBm), tested in loopback. The mid-packet (guard-interval correlation) detector is tested on this transmitter's waveform only; its false-alarm rate on real interference is unmeasured |
...
| Feature | Status | Evidence / gap |
|---|---|---|
| Data / RTS / Ack / A-MPDU wire formats, FCS, LLC/SNAP | External | Frames from real chips parse and re-serialize byte-exact |
...
| Pluto iiod client | Self-consistent (libiio semantics) | The client is exercised against the virtual Pluto, which reproduces iiod's READBUF framing from the libiio sources (mask line once per OPEN, no trailing 0 on full reads); a real Pluto is still the only proof |
...
| Good-neighbor frame filter | Self-consistent | Unit tests on crafted IPv4/IPv6/ICMPv6/extension-header/tunnel/ESP frames; not yet exercised against a real OS stack, so a protocol that hides behind an unlisted port or EtherType simply passes (deny list above the IPv6/EtherType level). ESP-NULL recognition is the RFC 5879 heuristic: an ESP-NULL stack using non-default padding would be dropped as encrypted |
| Per-peer rate control (MCS, guard interval, LDPC) | Self-consistent, simulated | Unit tests with synthetic success ceilings, a two-node PHY simulation (MCS at 30 dB / 11 dB SNR; short GI and LDPC adopted at 30 dB), and a link-level simulation (`tests/rate_sim.rs`) with the PHY's measured PER-vs-SNR curves, LDPC gain and delay-spread limits over static, shadowed, Rician/Rayleigh and stepped channels with and without delay spread and LDPC-capable peers, against which the constants were tuned (2026-09-03: 97 % of the best fixed MCS, GI and coding on average, 81 % worst case). The SNR table and the delay-spread limit are this PHY in loopback in the receiver's reported units; the hints assume a roughly symmetric link; LDPC support is inferred from an acknowledged LDPC frame; real fading statistics remain unmeasured |
...
| Partial AID for NDP CTS derived from the MAC address | No AID without association | Only matters for the RA field of s2g's own NDP CTS |
...
| Response waits of tens of ms (learned from measured turnarounds, 150 ms ceiling) instead of SIFS-scale | Buffered SDR streaming | Throughput bound; interop with a real SIFS-timed peer is not possible without hardware timestamping |
...
- **LDPC / traveling pilots TX**: run `scripts/matlab_vectors.m` (WLAN Toolbox) and decode the `.cf32` files with `s2g-rx --mac`; or capture any device that advertises LDPC.
- **NDP CMAC**: needs a device that sends NDP Ack/CTS; the Newracom NRC7292 software package has an NDP Ack mode; capture with a Pluto at 3.84 MS/s and look for `NdpReceived` in `s2g-rx` output.
- **MCS 8, 11**: any dev kit with rate control disabled, or MATLAB vectors.
- **RF conformance**: spectrum analyzer on the Pluto output; the `conformance` module then applies to a captured loopback as well.
